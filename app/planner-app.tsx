...
import { carrackGearEstimate, carrackGearSetEstimate, daysForUnits, estimateContext, formatDuration, formatRate, gearEstimate, materialEstimate, materialRate, questRatePerDay, shipEstimate } from "@/lib/estimate";
import { usePlannerStore } from "@/lib/store";
import type { EstimateContext, MaterialEstimate } from "@/lib/estimate";
import type { Acquisition, AcquisitionType, CarrackTarget, GearKey, MaterialCategory, MaterialId, PlannerPreset, PlannerProfile, ShipBranch } from "@/types";
...
function materialEtaLine(profile: PlannerProfile, id: MaterialId, context: EstimateContext) {
  ...
  return `${etaText(estimate.days)} no ritmo de ${formatRate(materialRate(profile, id, context).perDay)}${coins}`;
}
...
function AcquisitionCatalog() {
  ...
  return <><Header title="Materiais e onde conseguir" subtitle="Missões, compra com Moeda Corvo, processamento, drops e permuta em uma única lista, com o ritmo diário que cada fonte rende." />
    ...
    <div className="acquisition-list">{materials.map((m) => { const required = getRequired(m.id, profile.target); const missing = getMissing(profile, m.id); const sources = m.sources.filter((s) => sourceMatches(s, filter)); const rate = materialRate(profile, m.id, context); const estimate = materialEstimate(profile, m.id, context); return <article className="acquisition-card" key={m.id}><div className="acquisition-item"><MaterialLabel id={m.id} size={42} /><div className="acquisition-numbers"><span>Meta <strong>{required ? number(required) : "Aprimoramento"}</strong></span><span>Tenho <strong>{number(profile.materials[m.id])}</strong></span><span>Falta <strong className={missing === 0 ? "ok-text" : "warn-text"}>{required ? number(missing) : "—"}</strong></span><span>Ritmo <strong>{formatRate(rate.perDay)}</strong></span>{estimate.covered > 0 && <span>Com moedas <strong>{number(estimate.covered)}</strong></span>}<span>Tempo <strong>{required ? materialEtaText(estimate) : "—"}</strong></span></div></div><div className="used-in"><span>USADO EM</span>{usedIn(m.id).map((label) => <small key={label}>{label}</small>)}</div><div className="acquisition-sources">{sources.map((source, index) => { const perDay = questRatePerDay(source, context.competitors); return <div className={`source-card source-${source.type}`} key={`${source.type}-${index}`}><Badge kind={source.type}>{sourceLabel[source.type]}</Badge><strong>{source.label}</strong>{source.detail && <p>{source.detail}</p>}{perDay > 0 && <small className="source-rate">Rende ≈ {formatRate(perDay)} para este material</small>}{perDay === 0 && source.type !== "crow" && rate.farmPerDay > 0 && <small className="source-rate">Entra no ritmo como ≈ {formatRate(rate.farmPerDay)}, contados uma vez entre as rotas livres deste material</small>}{source.type === "crow" && estimate.covered > 0 && <small className="source-rate">Seu saldo cobre {number(estimate.covered)} un. agora e encurta o prazo</small>}</div>; })}</div></article>; })}</div>
  </>;
}
